# البرومبت الشامل لإصلاح منصة سوق لعوينات العملاق

> هذا برومبت واحد شامل: المتطلبات الكاملة (25 قسمًا) + كل كود المشروع الحالي بعد الإصلاحات المطبقة + التعليمات النهائية.
> انسخه كاملاً كما هو والصقه في Google AI Studio (أو أي مساعد ذكاء اصطناعي).
> حجمه كبير لأن الكود كله مضمّن — لا تقصّه أبدًا، فكل جزء له دور.

---


أنت تعمل الآن على مشروع موجود بالفعل اسمه:
...
لا تغير إطار العمل.
لا تغير package.json إلا إذا كان هناك خطأ يمنع التشغيل.
لا تستبدل Firebase بنظام آخر.
لا تضف بنية مشروع جديدة غير موجودة إلا إذا كانت ضرورية فعلًا.

المشروع الحالي React + TypeScript + Vite + Firebase.

الملفات الأساسية الموجودة في المشروع تشمل:
src/App.tsx
src/main.tsx
src/firebase.ts
...
المطلوب الآن هو إصلاح المشروع الحالي بالكامل ليعمل فعليًا في Production وليس فقط داخل Preview.

==================================================

أولًا: افحص المشروع قبل تعديل أي ملف

==================================================

اقرأ الملفات الحالية كاملة.
حدد:
1. بنية React
...
لا تعدل قبل فهم العلاقة بين هذه الملفات.
...
لا تستبدل هذه المنظومة.
تأكد من أن db و auth و storage تعمل في Production.
لا تستخدم Mock Firebase.
لا تستخدم localStorage بدل Firestore للبيانات الأساسية.
...
يجب ألا يستطيع المستخدم العادي تغيير role أو status لنفسه.
احذف Facebook Login بالكامل.
لا تعرض Facebook في واجهة تسجيل الدخول.

المستخدم يستطيع تعديل معلوماته الشخصية المسموحة فقط
(الاسم، الهاتف، الواتساب — بدون الرتبة والحالة).
...
حساب المدير الأساسي: bayssahmr@gmail.com
يجب أن يستطيع المدير الوصول إلى AdminDashboard.
لا تعتمد على إخفاء زر المدير فقط، يجب التحقق من الصلاحية فعليًا.

الأدوار: OWNER / SUPER_ADMIN / ADMIN / USER
المستخدم USER لا يستطيع الدخول إلى لوحة الإدارة حتى لو كتب /admin يدويًا.
...
الإعلانات موجودة في listings.
يجب أن تعمل create / read / update / delete فعليًا في Firestore.

عند نشر إعلان:
1. تحقق من تسجيل الدخول.
2. أنشئ بيانات الإعلان.
3. لا ترسل أي undefined إلى Firestore.
4. احفظ الإعلان.
5. أظهر رسالة نجاح.
6. حدّث قائمة الإعلانات.

==================================================

سادسًا: إصلاح undefined
...
found in field whatsapp

يجب إصلاح السبب على مستوى المشروع كله.
ابحث عن جميع الأماكن التي يمكن أن ترسل undefined إلى:
setDoc / addDoc / updateDoc

خصوصًا: whatsapp / phone / price / location / image / description / ownerName / categoryId / sellerId

استخدم null أو '' أو احذف الحقل، لكن لا ترسل undefined إلى Firestore.
استخدم دالة تنظيف مركزية واحدة.
...
ثم تخزين السعر كرقم صحيح.
وعرضه: 28,000 دج أو 28.000 دج بحسب formatter موحد.
لا تمنع نشر الإعلان بسبب تنسيق غير ضروري.

==================================================

ثامنًا: نموذج نشر الإعلان

==================================================

اجعل النموذج بسيطًا.
لا تجعل كل الحقول إجبارية.
الحقول الضرورية فقط: العنوان، الفئة، الوصف، الموقع، وسيلة اتصال مناسبة.
بقية المعلومات اختيارية حسب نوع الإعلان.
لا تمنع نشر إعلان بسبب عدم إدخال WhatsApp.
...
استخدم Firebase Storage الحالي.
الأنواع: PNG / JPG / JPEG / WEBP / SVG
الحد الأقصى: 10MB
...
2. تحقق من الحجم.
3. ارفع الملف.
4. احصل على download URL.
5. احفظ الرابط في Firestore.

لا تستخدم blob URL كرابط دائم.
...
AdminDashboard يجب أن تكون مركز التحكم الشامل.
أقسامها:
الرئيسية
المستخدمون
البائعون
...
لا تحذف الأقسام الموجودة حاليًا.

==================================================

الحادي عشر: إعدادات الهوية

==================================================

داخل PlatformCustomizationSettings يجب أن يستطيع المدير تغيير:
اسم المنصة
الوصف
الألوان
الشعار
...
ويجب حفظها فعليًا في settings/platform باستخدام updatePlatformSettings().

==================================================

الثاني عشر: مشكلة زر الحفظ

==================================================

لا تسمح بأن يبقى "حفظ وجار النشر الآن..." بلا نهاية.
كل عملية حفظ يجب أن تحتوي على: loading / success / error / finally.
عند النجاح: "تم حفظ التغييرات بنجاح"
عند الفشل: "فشل حفظ التغييرات" مع تسجيل الخطأ الحقيقي في console.
ضع timeout منطقيًا للعمليات التي قد تتعلق بالرفع أو الشبكة.

==================================================

الثالث عشر: الصور في لوحة الإدارة

==================================================

عند استبدال Logo / Favicon / OpenGraph الترتيب:
1. رفع الصورة الجديدة
2. التأكد من نجاح الرفع
3. الحصول على URL
4. حفظ URL الجديد في Firestore
5. تحديث الواجهة
6. حذف الصورة القديمة إن أمكن

إذا فشل رفع الجديدة: لا تحذف القديمة.

==================================================

الرابع عشر: OpenGraph

==================================================

المنصة: سوق لعوينات العملاق
الوصف: أول منصة إعلانات مبوبة محلية مخصصة لسكان العوينات وولاية تبسة.

يجب دعم: og:title / og:description / og:image / og:url
لا تعتمد فقط على React بعد تحميل الصفحة.
اجعل metadata الموجودة في index.html صحيحة كقيم افتراضية.

==================================================

الخامس عشر: التسويق بالعمولة

==================================================

القسم الموجود باسم "المتجر" غيّر اسمه إلى "التسويق بعمولة".
عند الضغط عليه افتح مباشرة: https://sawa9ly.app/?r=97834
لا تفتح صفحة داخلية.
لا تضف utm_source.
استخدم رابط الإحالة الأصلي فقط.

==================================================

السادس عشر: الإعلانات التجريبية

==================================================

لا تنشئ بيانات وهمية تلقائيًا إذا كانت قاعدة البيانات فارغة.
المنصة يجب أن تعتمد على Firestore.
لا تزرع بيانات اختبار تلقائيًا في Production.
...
حذف الإعلان يجب أن يستخدم deleteDoc() فعليًا.
بعد نجاح الحذف: حدّث الواجهة.
إذا فشل الحذف: لا تحذف الإعلان من state وأظهر رسالة خطأ.

==================================================

الثامن عشر: Firestore Rules

==================================================

راجع firestore.rules الموجود.
يجب السماح:
- قراءة الإعلانات للجميع.
- إنشاء الإعلان للمستخدم المسجل فقط.
- إنشاء الإعلان فقط إذا request.resource.data.ownerId == request.auth.uid
- التعديل والحذف: لصاحب الإعلان أو المدير.
- المستخدم لا يستطيع تغيير ownerId بعد إنشاء الإعلان.
- إعدادات المنصة: للمدير فقط.
- التصنيفات: للمدير فقط عند الكتابة.
- البنرات: للمدير فقط عند الكتابة.
- المستخدم لا يستطيع منح نفسه ADMIN أو OWNER.
...
حافظ على manifest.webmanifest و sw.js.
اجعل name / short_name / start_url / display / icons صحيحة.
يجب توفير أيقونات مناسبة.
لا تستخدم favicon صغيرة كأنها 512x512.
Service Worker يجب ألا يتسبب في عرض نسخة HTML قديمة بلا نهاية.
استخدم استراتيجية Cache مناسبة (Network First).
...
أي شيء يعتمد على localhost يجب ألا يستخدم في Production.

==================================================

الحادي والعشرون: أخطاء الشبكة

==================================================

كل Firebase call يجب أن يحتوي على try / catch / finally.
لا تجعل أي زر يبقى loading إلى الأبد.

==================================================

الثاني والعشرون: لا تغيّر البنية

==================================================

لا تنشئ مشروعًا جديدًا.
لا تغير React / Vite / Firebase / TypeScript.
لا تحذف الملفات الموجودة إلا إذا كانت مكررة أو تسبب مشكلة مثبتة.

==================================================

الثالث والعشرون: الاختبار
...
الرابع والعشرون: قاعدة مهمة جدًا

==================================================

لا تقل "تم الإصلاح" إذا لم تتحقق من الكود.
إذا وجدت مشكلة لا يمكن حلها من داخل الكود، اذكرها صراحة.
مثال: Firebase Storage يحتاج Rules. أو Authorized Domain يحتاج إضافة.
أو Firebase Authentication Provider يحتاج تفعيل.
لا تخفي المشكلة.

==================================================

الخامس والعشرون: النتيجة المطلوبة

==================================================

أريد منك تعديل المشروع الحالي نفسه.
بعد الانتهاء اعرض لي:
قائمة الملفات التي عدلتها.
أهم التغييرات.
الأخطاء التي تم إصلاحها.
أي إعداد Firebase خارجي ما زال مطلوبًا.

لا تعيد كتابة المشروع من الصفر.
لا تنشئ Demo.
لا تستخدم بيانات وهمية بدل Firebase.
اجعل النسخة النهائية قابلة للعمل في Production.

==================================================

السادس والعشرون: الكود الحالي الكامل بعد الإصلاحات المطبقة

==================================================

فيما يلي كل ملفات المشروع الحالية كما هي الآن (بعد الإصلاحات المطبقة فعلاً).
اقرأها جميعاً قبل أي تعديل، وعدّل في نفس هذه الملفات — لا تعيد كتابتها من الصفر.
الملفات مرتبة حسب البنية. كل ملف محاط بعلامتي بداية ونهاية.
...
===== FILE: src/data/categoriesData.ts =====
...
===== FILE: src/components/AdminLoginPage.tsx =====
import React, { useState } from 'react';
...
===== FILE: src/components/ListingDetailModal.tsx =====
import React, { useState } from 'react';
...
===== FILE: src/components/PlatformCustomizationSettings.tsx =====
